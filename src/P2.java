...
public class P2 {
	public static void main(String[] args) throws IOException {
		...
		while (sc.hasNextLine()) {
			...
			String[] tunnels = Arrays.stream(s.split(",")).map(str -> str.split(" ")[str.split(" ").length-1]).toArray(String[]::new);
					...
					for (int x : al.get(i-1).get(s).get(s2).keySet()) {
						...
						for (String t : g.get(s)) {
							for (String t2 : g.get(s2)) {
								al.get(i).get(t).get(t2).put(x, Math.max(nv, al.get(i).get(t).get(t2).getOrDefault(x,0)));
						...
						if (flows.get(s)!=0) {
							...
							for (String t2 : g.get(s2)) {
								al.get(i).get(s).get(t2).put(nx, Math.max(nv, al.get(i).get(s).get(t2).getOrDefault(nx,0)));
						...
						if (flows.get(s2)!=0) {
							...
							for (String t : g.get(s)) {
								al.get(i).get(t).get(s2).put(nx, Math.max(nv, al.get(i).get(t).get(s2).getOrDefault(nx,0)));
								ans = Math.max(ans, al.get(i).get(t).get(s2).get(nx));
							}
						}
						if (flows.get(s)!=0&&flows.get(s2)!=0) {
							int nx = x|(1<<flows.get(s))|(1<<flows.get(s2));
							al.get(i).get(s).get(s2).put(nx, Math.max(nv, al.get(i).get(s).get(s2).getOrDefault(nx,0)));
